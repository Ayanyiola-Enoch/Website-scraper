...
#     html = fetch_page_content(url)
#     if not html:
#         return {"title": "Failed to load", "content": "Could not fetch page content"}
...
# # Function to extract title and practice areas with details
# def extract_data(html, url):
#     soup = BeautifulSoup(html, "lxml")
...
#             # Save practice areas as CSV
#             df_practice_areas = pd.DataFrame(practice_areas_data)
#             df_practice_areas.to_csv(csv_file, index=False)
            
#             print(f"Data saved to {csv_file} due to Excel error")
#             return True
#         except Exception as csv_err:
#             print(f"Failed to save even as CSV: {csv_err}")
#             return False
...
#         if data["practice_areas"]:
#             if save_to_excel(data):
#                 print(f"Scraping completed successfully! Found {len(data['practice_areas'])} practice areas.")
#             else:
#                 print("Scraping completed but there were issues saving the data.")
#         else:
#             print("No practice areas found on this website.")
